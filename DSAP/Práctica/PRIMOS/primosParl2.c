...
int main(int argc, char *argv[])
{
    ...
    double t0, t1, tiempo_secuencial, tiempo_parcial;
    int rank, num_procs;
    ...
    while (n <= n_max)
    {
        ...
        t1 = MPI_Wtime();
        tiempo_parcial = t1 - t0;

        MPI_Reduce(&tiempo_parcial, &t1, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);

        if (rank == 0)
        {
            printf(" Primos menores que %10d: %10d.\n Tiempo secuencial: %5.2f segundos.\n Tiempo parcial del proceso %d: %5.2f segundos.", n, primos, tiempo_secuencial, rank, tiempo_parcial);
            printf("\nTiempo total paralelo: %5.2f segundos.\nSpeedup: %5.2f. Eficiencia: %5.2f\n", t1, tiempo_secuencial / t1, (tiempo_secuencial / t1) / num_procs);
            printf("\n-----------------------------------------------\n");
        }
        n = n * n_factor;
    }
