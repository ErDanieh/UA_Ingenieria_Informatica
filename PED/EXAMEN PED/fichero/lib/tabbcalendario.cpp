...
/**
 * @brief Función del examen le pasamos una lista y devuelve un vector
 * 
 * @param camino 
 * @return TVectorCalendario 
 */
TVectorCalendario TABBCalendario::ABBCamino(TListaCalendario &camino)
{
    TListaPos posicion;
    TCalendario estoyLista;
    TNodoABB *punteroNodo;
    bool estaAbb;
    TVectorCalendario salida;
    //Si el arbol que tenemos es vacio se devuelve un vector vacio
    ...
    {
        posicion = camino.Primera();
        //La posición de la lista no puede ser vacía
        while (posicion.EsVacia() == false)
        {
            estoyLista = camino.Obtener(posicion);
            //Si no lo encontramos en el árbol y además el calendario que nos pasan
            //No es vacio
            if (Buscar(estoyLista) == false && estoyLista.EsVacio() == false)
            {
                //insertamos el nodo en el arbol
                Insertar(estoyLista);
                punteroNodo = this->raiz;
                estaAbb = false;
                //Buscamos el nodo
                while (estaAbb == false)
                {
                    //Redimensionamos el vector para no tener huecos en blanco
                    salida.Redimensionar(salida.Tamano() + 1);
                    //Añadimos el item al vector como es un calendario no hay problemas pero hay que hacer
                    //El puntero al item
                    salida[salida.Tamano()] = punteroNodo->item;
                    //Ahora las condiciones de izquierda y derecha de ABB más grande derecha y más pequeño izquierda
                    if (estoyLista == punteroNodo->item)
                        estaAbb = true;
                    else if (estoyLista > punteroNodo->item)
                        punteroNodo = punteroNodo->de.raiz;
                    else
                        punteroNodo = punteroNodo->iz.raiz;
                }
            }
            //Pasamos a la siguiente posición de la lista hasta que se quede vacia
