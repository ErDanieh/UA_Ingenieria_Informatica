      ...
      loop
         select
            ...
            then
               ...
               -- Comprobamos que la siguiente aerovia no es la ultima, y si lo es, eliberamos la rejilla actual, desaparecemos y apagamos el proceso hijo.
               if avion.aereovia + 1 = T_Rango_AereoVia'Last then
                  arrayAerovias (avion.aereovia).librar(Posicion_Rejilla (avion.pos.X));
                  Desaparece (avion);
                   Escribir
                    ("  - Aterrizando avion:" & avion.id'Image &
